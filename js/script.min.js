...
$(document).ready(function() {
    $('[data-tab="html"]').click(function() {
        $('[data-type="html"]').css('display', 'grid');
        $('[data-type="js"]').css('display', 'none');
        $('[data-type="other"]').css('display', 'none');
        $('[data-tab="html"]').addClass('portfolio-active');
        $('[data-tab="js"]').removeClass('portfolio-active');
        $('[data-tab="other"]').removeClass('portfolio-active');
    });
    $('[data-tab="js"]').click(function() {
        $('[data-type="html"]').css('display', 'none');
        $('[data-type="js"]').css('display', 'grid');
        $('[data-type="other"]').css('display', 'none');
        $('[data-tab="html"]').removeClass('portfolio-active');
        $('[data-tab="js"]').addClass('portfolio-active');
        $('[data-tab="other"]').removeClass('portfolio-active');
    });
    $('[data-tab="other"]').click(function() {
        $('[data-type="html"]').css('display', 'none');
        $('[data-type="js"]').css('display', 'none');
        $('[data-type="other"]').css('display', 'grid');
        $('[data-tab="html"]').removeClass('portfolio-active');
        $('[data-tab="js"]').removeClass('portfolio-active');
        $('[data-tab="other"]').addClass('portfolio-active');
    });

    $('.hamburger').click(function() {
        let visible = $('.menu__block').css('top');
        if (visible === '-246px') {
            $('.menu__block').css('top', '66px');
            $('.hamburger span:first-child').css({ 'transform': 'rotate(45deg) translateX(2px)', });
            $('.hamburger span:nth-child(2)').css({ 'display': 'none', });
            $('.hamburger span:nth-last-child(1)').css({ 'transform': 'rotate(-45deg) translateX(2px)', 'margin': '0', });
        } else {
            $('.menu__block').css('top', '-246px');
            $('.hamburger span:first-child').css({ 'transform': 'none', });
            $('.hamburger span:nth-child(2)').css({ 'display': 'block', });
            $('.hamburger span:nth-last-child(1)').css({ 'transform': 'none', 'margin': '7px 0', });
        }
    });
    const mediaMobile = window.matchMedia('(min-width: 769px)');
    let portfolioItem = document.querySelectorAll('.portfolio__grid-item');
    if (mediaMobile.matches) {
        portfolioItem.forEach(function(item) {
            $(item).hover(function() {
                $(item).children('div').css({ 'display': 'block', });
                $(item).children('img').css({ 'filter': 'brightness(30%)' });
            }, function() {
                $(item).children('div').css({ 'display': 'none', });
                $(item).children('img').css({ 'filter': 'brightness(100%)' });
            });
        });
    } else {
        portfolioItem.forEach(function(item) {
    ...
    close.addEventListener('click', (e) => { overlay.style.display = 'none'; });
